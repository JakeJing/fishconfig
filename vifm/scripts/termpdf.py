...
# it works on python3.11, not working on python3.9 or 3.10
...
class Config:
    ...
    def load_config_file(self):
        config_file = os.path.expanduser(
            os.path.join(
                os.getenv("XDG_CONFIG_HOME", "~/.config"), "termpdf.py", "config"
            )
        )
...
class Document(fitz.Document):
    ...
    def __init__(
        self, filename=None, filetype=None, rect=None, width=0, height=0, fontsize=12
    ):
        ...
        self.logical_pages = list(
            range(0 + self.first_page_offset, self.pages + self.first_page_offset)
        )
        self.chapter = 0
        self.rotation = 0
